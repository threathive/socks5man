...
class DbManager:
    ...
    def add_server(self, server):
        ...
        query = ("INSERT INTO server"
                 " (server_id,ip,port,username,password,country,last_checked, last_used, active)"
                 " VALUES (?,?,?,?,?,?,?,?,?)")
        try:
            self.cursor.execute(query,(server.identifier,
                                       server.ip,
                                       server.port,
                                       server.username,
                                       server.password,
    ...
    def update_server_stats(self, server):
        ...
        success = False
        query   = "UPDATE server SET last_checked = ?, last_used = ?, active=? WHERE server_id = ?"
        
        try:
            self.cursor.execute(query, (server.last_checked,
                                        server.last_used,
                                        server.active,
                                        server.identifier))
            self.conn.commit()
    ...
    def get_longest_unused_server(self):
        ...
        query = "SELECT * FROM server WHERE active = 1 ORDER BY last_used ASC LIMIT 1"
